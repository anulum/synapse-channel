...
是修复和加固，而不是折腾。wire 协议和公开的 Python API 在主版本内保持向后
兼容；任何破坏性变更都会在 changelog 中说明。

当前的 `0.x` 发行是开发发行，不是稳定的商业发行线。SYNAPSE CHANNEL 的
第一个稳定商业发行已在计划中，其运营契约、打包、支持面和商业许可条款将
作为该发行的一部分写入文档。

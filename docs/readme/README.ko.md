...
수정과 강화입니다. 현재 `0.x` 릴리스는 마이너 릴리스 간 하위 호환성을
보장하지 않습니다. 와이어 어휘와 공개 Python API는 의도치 않은 드리프트를
막는 테스트로 보호되지만, 검토된 `0.x` 마이너 릴리스에서 의도적으로 바뀔 수
있습니다. 이런 변경은 체인지로그와 마이그레이션 노트에 기록하며, 와이어
비호환 변경은 `WIRE_PROTOCOL_VERSION`을 올립니다. `1.0.0`부터 안정 공개
Python API의 파괴적 변경에는 새로운 패키지 메이저 릴리스가 필요합니다.
[API 및 와이어 안정성](../api-stability.md)을 참고하세요.

`1.0.0`은 SYNAPSE CHANNEL의 첫 안정 상용 릴리스로 계획되어 있으며, 운영

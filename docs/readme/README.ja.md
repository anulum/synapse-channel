...
ほとんどが小さいもの — チャーンではなく修正と強化です。ワイヤプロトコルと
公開 Python API はメジャーバージョン内で後方互換を保ち、破壊的変更はすべて
changelog に明記されます。

現在の `0.x` リリースは開発リリースであり、安定した商用リリースラインでは
ありません。SYNAPSE CHANNEL の最初の安定商用リリースが計画されており、

...
`ProviderHTTPClient` returns tool calls as data. It accepts only a list of tool-call objects in a non-stream provider response and reports malformed collections as `invalid_shape` without exposing provider content. It never runs tools, retries a turn, or executes model output. The synchronous worker asks for one non-stream completion; its lower-level client can request SSE and normalizes the buffered result. Cancellation checks before and during reads, bounded by the socket timeout; provider-side cancellation acknowledgement is unknown. Rate headers are returned in `ProviderReply`; the worker does not yet adapt its scheduling to them. A quoted API price never implies a current account quota.

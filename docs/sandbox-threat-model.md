...
| **Symlink-swapped preopen / escape outside the workspace** | Each grant's host path is canonicalised before the preopen: a path that resolves through a symlink, or is not an existing directory, is refused, and the resolved real path is what is preopened and recorded — a link swapped in after the manifest was authored cannot redirect the sandbox. An operator may also pass `--workspace-root DIR` (repeatable): every preopen must then resolve at or below an approved root, so a genuine but out-of-workspace directory is refused too. | Fail-closed — the run is refused (`EXIT_ERROR`, "sandbox path refused") before the tool executes; `synapse sandbox validate --check-paths [--workspace-root DIR]` reports the same verdict without running anything. |

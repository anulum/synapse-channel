...
immutable. SYNAPSE CHANNEL is active there: version `0.99.21` was the latest
public record when last re-verified; `0.99.22` is the in-tree preparation tip
...
1. Release `synapse-channel==0.99.22` to PyPI through the normal attested tag
   ...
   PYTHONPATH=. .venv/bin/python tools/verify_mcp_registry_release.py \
     --phase package --expect-version 0.99.22 --json
   ```
...
4. Require the public record itself to prove completion:
   ...
   ```bash
   PYTHONPATH=. .venv/bin/python tools/verify_mcp_registry_release.py \
     --phase registry --expect-version 0.99.22 --json
   ```

...
The verified contract is **OpenCode 1.18.32**, official tag commit
`545f51d26cc39a907d2867492d498d9607ea5fa4`. Both participant drivers refuse a
different server or CLI version instead of attempting to parse an unknown
schema.

The 1.18.32 review checked the official Linux x64 archive and extracted
binary, then ran JSONL, ACP, authenticated server and claim-guard journeys
against a local scripted provider and real Synapse hub. The previous 1.18.31
...
1.18.32, creates or resumes a session, posts a bounded text prompt, and maps the
...
OpenCode 1.18.32's non-interactive `run --attach` path posts the prompt but
...
OpenCode 1.18.32 exposes an ACP subprocess:
...
credentials. Each lane then requires CLI version 1.18.32 and performs a real ACP
initialize exchange:
...
by SHA-256, verifies exact version 1.18.32, builds the wheel, checks its OpenCode
...
- [OpenCode 1.18.32 release](https://github.com/anomalyco/opencode/releases/tag/v1.18.32)

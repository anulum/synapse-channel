...
`arm`  `board`  `channel`  `commands`  `completions`  `demo`  `hub`  `init`  `listen`  `lock`  `new`  `quickstart-coding`  `send`  `status`  `task`  `team`  `wait`  `who`
...
`a2a-card`  `a2a-serve`  `adapters`  `agent-tmux`  `codex-tmux`  `git-claim`  `git-hook`  `git-init`  `git-release`  `ingest`  `install-shell-hook`  `mcp`  `mcp-call`  `mcp-tools`  `shell-hook`  `worker`  `worker-session`
...
`accounting`  `causality`  `conflicts`  `cross-repo`  `dashboard`  `debug`  `directory`  `doctor`  `event-query`  `health`  `identity`  `manifest`  `multihub`  `relay`  `reliability`  `state`  `trust-graph`
...
`acl`  `approval`  `compact`  `encrypt-key`  `federation`  `merkle`  `policy-check`  `postmortem`  `release`  `reproduce`  `supervisor`  `verify-release`
...
`benchmark`  `memory-recall`  `participant`  `resource-bids`  `route-task`  `sandbox`  `ttl-advice`
`workflow`

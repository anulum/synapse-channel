...
def test_wait_and_wake_retries_failed_wait_with_backoff_then_wakes(tmp_path: Path) -> None:
    ...
    # Two backoff sleeps for the two failures, then one submit-delay sleep for the wake.
...
def test_wait_and_wake_resets_failure_counter_after_a_wake(tmp_path: Path) -> None:
    ...
    assert result == 0
    # Each failure backs off from base because the counter resets after each wake.

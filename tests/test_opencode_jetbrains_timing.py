...
def test_default_timing_budget_covers_every_driver_phase_and_cleanup() -> None:
    ...
    assert budget.driver_budget_seconds == 800.0
    assert budget.parent_timeout_seconds == 920
...
def test_driver_consumes_the_shared_timing_budget() -> None:
    ...
    assert jetbrains_evidence._SCREENSHOT_TIMEOUT_SECONDS == budget.screenshot_seconds
...
def test_gui_commands_cannot_overrun_their_absolute_phase_deadline(
    ...
    assert jetbrains_x11_driver._command_timeout(None) == 10.0
    assert jetbrains_x11_driver._command_timeout(120.0) == 10.0
    assert jetbrains_x11_driver._command_timeout(103.5) == 3.5
    with pytest.raises(RuntimeError, match="phase deadline expired"):
        jetbrains_x11_driver._command_timeout(100.0)
...
def test_poll_sleep_cannot_cross_the_absolute_phase_deadline(
    ...
    jetbrains_x11_driver._bounded_poll_sleep(100.0)
    jetbrains_x11_driver._bounded_poll_sleep(100.1)
    jetbrains_x11_driver._bounded_poll_sleep(101.0)

...
# The real-wheel journey must prove the console scripts load from the INSTALLED
# wheel, so its subprocesses must not inherit an ambient ``PYTHONPATH`` pointing at
# the working tree's ``src``. Another test module in this suite sets that on the
# process environment for its own CLI journeys, and a subprocess would otherwise
# import ``synapse_channel`` from the source tree and the gate would spuriously
# report every script as "loaded outside site-packages". Strip it for a clean env.
_WHEEL_ENV = {key: value for key, value in os.environ.items() if key != "PYTHONPATH"}

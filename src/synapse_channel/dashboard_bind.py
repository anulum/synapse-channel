...
def _resolve_dashboard_token(
    ...
    *,
    ...
    tuple[str or None, bool, bool]
        ...
        whether it also protects reads (``GET``). A caller-supplied token and a token
        generated for an exposed bind protect reads; a token generated only to gate
        operator writes on loopback does not.
    ...
    if dashboard_token is not None:
        # An operator-supplied token is a deliberate access control on the whole
        # surface, so it gates reads and writes alike.
        return dashboard_token, False, True
    if allow_non_loopback and not _is_loopback_host(host):
        # An exposed bind must authenticate every request, reads included.

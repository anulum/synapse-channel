...
def _resolve_dashboard_token(
    ...
    *,
    ...
    operator : bool
        Whether the operator write-path is armed. When it is, a token is required for
        *writes* even on loopback: an unauthenticated write endpoint is a same-host
        cross-process (local CSRF) write risk, so a token is generated if the caller
        supplied none. That token gates only writes — reads stay open on loopback so
        the read-only browser cockpit still loads (a browser cannot send an
        ``Authorization`` header on navigation, so gating reads would lock it out).

    Returns
    -------
    tuple[str or None, bool, bool]
        The effective bearer token, whether it was generated by the server, and
        whether it also protects live/page reads (``GET``). A caller-supplied token
        and a token generated for an exposed bind protect those reads; the validated
        React cockpit shell remains the narrow exception so its unlock veil can load.
        A token generated only to gate operator writes on loopback does not protect
        reads.
    ...
    if allow_non_loopback and not _is_loopback_host(host):
        # An exposed bind must authenticate every live/page request, reads included.
        return secrets.token_urlsafe(32), True, True
    if operator:
        # Loopback operator writes: gate writes with a generated token, but leave reads
        # open so the browser cockpit still loads.

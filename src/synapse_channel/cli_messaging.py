...
These three commands connect to a live hub and exchange chat with it, as opposed
...
— the wake primitive an agent re-arms in the background; ``listen`` streams chat
and presence until interrupted. They are grouped here, apart from the hub and
query flows, so each module stays one responsibility; :func:`add_parsers`

...
remainder range. Declarations outside the bounded model are listed as
unassessed rather than silently skipped, and the advice is text for the
operator: nothing here rewrites a manifest.
"""
        ...
        if item.odd_ones_out:
            for odd in item.odd_ones_out:
                lines.append(
                    ...
                    f"the other declarations reconcile at {odd.remainder}"
                )
        else:
            lines.append(
